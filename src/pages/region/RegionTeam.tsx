...
import { usePeople } from '@/hooks/useSupabaseData';
...
export function RegionTeam() {
  ...
  const team = people.filter(p => p.region === regionId);
  ...
  return (
    <div className="w-full bg-plimsoll min-h-screen pb-20">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-16">
        {isLoading ? (
          <div className="py-24 text-center text-steel">Loading team profiles...</div>
        ) : error ? (
          <div className="py-24 text-center text-red-500">Failed to load team profiles.</div>
        ) : (
          <>
            ...
            {team.length === 0 && (
              <p className="text-deck-grey text-lg mt-8 text-center">Team profiles are currently being updated.</p>
            )}
          </>
        )}
      </div>
